...
class Pointerify {
  ...
  public movePointer(
    pointer: Pointer,
    e: IMouseOrTouchEvent = null,
    originalEvent: IMouseOrTouchEvent = null
  ): void {
    ...
    if (!pointer.isMoving) {
      ...
      if (
        (allowAxis === Axis.X && vector < 1) ||
        (allowAxis === Axis.Y && vector >= 1)
      ) {
  ...
  private stopPointer(pointer: Pointer): void {
    ...
    const render = () => {
      ...
      if (
        Math.abs(newVelocityX) < STOPPED_PXPF &&
