...
class Pointer {
  ...
  public get velocityX() {
    return this.velocitiesX.length &&
      this.pointerify.config.behavior.allowAxis !== Axis.Y
      ? this.velocitiesX.reduce((value, sum) => value + sum, 0) /
  ...
  public get velocityY() {
    return this.velocitiesY.length &&
      this.pointerify.config.behavior.allowAxis !== Axis.X
      ? this.velocitiesY.reduce((value, sum) => value + sum, 0) /
  ...
  private get deltaX() {
    return this.pointerify.config.behavior.allowAxis === Axis.Y
      ? 0
      : this.currentX - this.startX;
  }

  private get deltaY() {
    return this.pointerify.config.behavior.allowAxis === Axis.X
      ? 0
  ...
  private get velocityPinch() {
    return this.velocitiesPinch.length
      ? this.velocitiesPinch.reduce((value, sum) => value + sum, 0) /
  ...
  private getState() {
    ...
    state.multiplierX = clampX
      ? clamp(this.multiplierX, 0, 1)
      : this.multiplierX;
    state.multiplierY = clampY
      ? clamp(this.multiplierY, 0, 1)
      : this.multiplierY;
