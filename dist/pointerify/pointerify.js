...
var Pointerify = /** @class */ (function () {
    ...
    Pointerify.prototype.movePointer = function (pointer, e, originalEvent) {
        ...
        if (pointer.isVirtualPointer) {
            ...
            pointer.currentX =
                (pointer.yinPointer.currentX + pointer.yangPointer.currentX) / 2;
            pointer.currentY =
                (pointer.yinPointer.currentY + pointer.yangPointer.currentY) / 2;
        ...
        if (!pointer.isMoving) {
            ...
            var vector = Math.abs((pointer.currentX - pointer.startX) /
                (pointer.currentY - pointer.startY));
            if ((allowAxis === axis_1.default.X && vector < 1) ||
                (allowAxis === axis_1.default.Y && vector >= 1)) {
    ...
    Pointerify.prototype.stopPointer = function (pointer) {
        ...
        var render = function () {
            ...
            newVelocityX =
                directionX === direction_1.default.RIGHT
                    ? Math.max(0, newVelocityX)
                    : Math.min(0, newVelocityX);
            newVelocityY =
                directionY === direction_1.default.DOWN
                    ? Math.max(0, newVelocityY)
                    : Math.min(0, newVelocityY);
            if (Math.abs(newVelocityX) < STOPPED_PXPF &&
                Math.abs(newVelocityY) < STOPPED_PXPF) {
